...
import { nextEnrollmentRequestNumber } from "@/lib/identifiers";
...
const enrollmentRequestSchema = z.object({
  ...
  financialGuardianFirstName: z
    .string()
    .min(2, "Nome do responsável financeiro é obrigatório"),
  financialGuardianLastName: z
    .string()
    .min(2, "Sobrenome do responsável financeiro é obrigatório"),
...
export const POST = withoutAuth(async (request) => {
  try {
    ...
    // Número da solicitação pela sequência do Postgres. A versão anterior lia o
    // último registro e incrementava, o que repete o número sob concorrência e
    // zera o contador se o prefixo do último registro for outro.
    const requestNumber = await nextEnrollmentRequestNumber();

    // Criar solicitação
    // consentGiven is validated, not stored: the timestamp below is the record.
    const {
