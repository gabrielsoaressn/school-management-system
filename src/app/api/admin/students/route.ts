...
export const POST = withAuth(
  async (request) => {
    try {
      ...
      // Get system settings
      const autoGenerateBilling = await getSettingAsBoolean(
        "auto_generate_billing",
        true
      );
      const defaultTuition = await getSettingAsNumber(
        "default_tuition_monthly",
        1500
      );
      ...
      if (!studentCheck.valid) {
        return fail(studentCheck.errors[0], 400, {
          errors: studentCheck.errors,
        });
      }

      if (parentPassword) {
        const parentCheck = validatePassword(parentPassword, {
          email: parentEmail,
        });
        if (!parentCheck.valid) {
          return fail(parentCheck.errors[0], 400, {
            errors: parentCheck.errors,
          });
        }
      }
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (autoGenerateBilling && finalParentId) {
          ...
          const billingAmount = tuitionAmount || defaultTuition;
